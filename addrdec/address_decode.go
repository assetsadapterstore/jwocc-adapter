...
var (
	...
	JWOCCAddressPrefix_mainnet = "JWOCC"
	JWOCCAddressPrefix_testnet = "JWOCCT"
)

var (
	JWOCC_mainnetAddress = addressEncoder.AddressType{EncodeType: "base58", Alphabet: alphabet, ChecksumType: "ripemd160", HashType: "sha512_ripemd160", HashLen: 20, Prefix: []byte{0x35}, Suffix: nil}
...
func (dec *AddressDecoderV2) AddressDecode(addr string, opts ...interface{}) ([]byte, error) {
	...
	cfg := JWOCC_mainnetAddress

	addreesPrefix := JWOCCAddressPrefix_mainnet
	if dec.IsTestNet {
		addreesPrefix = JWOCCAddressPrefix_testnet
	}
...
func (dec *AddressDecoderV2) AddressEncode(hash []byte, opts ...interface{}) (string, error) {
	...
	addressPrefix := JWOCCAddressPrefix_mainnet
	if dec.IsTestNet {
		addressPrefix = JWOCCAddressPrefix_testnet
	}
	...
	if len(hash) != cfg.HashLen {
		...
		//8， 最终地址为固定字符串”JWOCC”+base58_data
